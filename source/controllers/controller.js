        ...
        if(result.errors.length > 0){
            ...
            .then(data=>{
                if(data.length>0){
                    ...
                    if(msg == ''){
                        const {frontIdCard,backIdCard,...data} = dataRegister;
                        accountModel.create({...account,...data,firstTimeLogin:true,balance:50000,status: "Waiting for verification",temporaryLock:false,note:'',historyRecharge:'',historyWithdraw:'',historyTransfer:'',timeWithdraw:"",transactionAmount:"",isAdmin:false,dateCreate,historyBuyCard:""});  
                        idCard.create({email: dataRegister.email,frontIdCard:dataRegister.frontIdCard,backIdCard:dataRegister.backIdCard})
            ...
            .then((data) => {
                if(data){
                    if(data[0].firstTimeLogin == true){
                        ...
                        res.render('home',{dataUser:data[0],balanceInAlph:new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(data[0].balance*1)});
                    }
                }
            })
        }else{
